...
test('server profile authorization uses Supabase users and active organization membership', () => {
  ...
  assert.match(profileSection, /\.eq\("active", true\)/);
...
test('server admin authorization requires both Firebase admin claims and an active tenant membership', () => {
  const adminSection = server.slice(server.indexOf('async function requireAdmin'), server.indexOf('async function getServerUserProfile'));
  assert.match(adminSection, /authContext\?\.admin/);
  assert.match(adminSection, /Active administrator membership required/);
